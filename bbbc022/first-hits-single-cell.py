...
def find_first_hits(features, feats, pert_name, control):
    ...
    gen = features[features[pert_name] != control].groupby(["Metadata_Plate", "Metadata_Well", pert_name])["Replicate_Use"].count().reset_index().iterrows()
    for k, r in tqdm(gen):
...
cols = ["Metadata_Plate","Metadata_Well","Metadata_Site","Plate_Map_Name","broad_sample_Replicate","Treatment","Compound","Concentration","Replicate_Use"]
...
def request_controls(control_meta_subset):
    ...
    return pd.merge(control_meta_subset, pd.DataFrame(data=sc_control_features_array), how="left", left_index=True, right_index=True)
...
def request_treatments(treatments_meta_subset):
    ...
    return pd.merge(treatments_meta_subset, pd.DataFrame(data=sc_treatment_features_array), how="left", left_index=True, right_index=True)
